...
- 🔀 **双模式创作引擎（Dual-Mode Workbench）**：
  - **💌 经典邮票模式**：四周齿孔物理打孔、满幅切图/留白调节、3款齿孔密度与6款纸张底衬。
  - **🎫 复古旅行票根模式**：支持 **「📱 竖版票根」** 与 **「💻 横版车票」** 双版式；上下/左右大圆角、凹槽撕裂线、圆角相片槽、站点名称 + 年月日 + 真实竖横条形码与票号。
- 📑 **批量选择多张照片制作**：支持一次性选取多张照片连续裁切，支持一键批量保存全部邮票/票根，或一键打包发布为小红书多图笔记。
- ✂️ **真实物理齿孔镂空算法**：基于 HTML5 Canvas `destination-out` 复合图层运算，生成逼真的圆弧边缘镂空与撕裂缺口。
- 🖼️ **满幅切图与留白自由调节**：
  - **满幅打孔（0 留白）**：齿孔直接咬合在照片边缘，画面满幅呈现。
  - **经典留白（14px+）**：模拟传统邮票与相框衬纸白边。
- 📐 **多画幅智能裁剪**：支持 `1:1`、`3:4`、`4:3`、`2:3`、`9:16` 常见比例构图，支持画面缩放、平移与一键旋转/回正（`0°` / `90°` / `180°` / `270°`）。
- 🎨 **多款经典齿孔预设**：提供标准齿、复古密齿、艺术大齿 3 款经典齿孔密度。
- 🖼️ **外层背景卡片与物理阴影**：支持将邮票/票根一键置入复古背景卡片中（暖黄纸、极夜黑、森林绿、灰蓝、落日橙、陶土红、柔雾粉等），自带真实立体柔光投影，小红书配图直出即用。
- 🌈 **个性化衬纸底色**：提供纯白、浅黄、青绿、粉红、浅蓝等多款纸张内衬。
